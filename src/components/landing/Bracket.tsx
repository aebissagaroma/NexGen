...
const ROUNDS = [
  { round: 'RD 256', date: 'JUL 11' }, { round: 'RD 64', date: 'JUL 13' },
  { round: 'RD 16', date: 'JUL 15' }, { round: 'QFs', date: 'JUL 16' }, { round: 'FINAL', date: 'JUL 17' },
];
      ...
      <div className="wrap">
        <div className="section-head" data-reveal>
          <div className="section-head-row">
            ...
            <p className="section-sub" style={{ alignSelf: 'flex-end' }}>Each of the 20 club brackets is a single-elimination knockout, BO3 throughout. Brackets are seeded and drawn live on broadcast the day after qualifier registration closes — <strong style={{ color: 'var(--ink)' }}>date announced nearer the time</strong>.</p>
          </div>
        </div>
        ...
        <div data-reveal style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', marginBottom: 24 }}>
          {ROUNDS.map((r, i) => (
            <div key={i} style={{ padding: '12px 14px', borderLeft: i === 0 ? '1px solid var(--line)' : 'none', borderRight: '1px solid var(--line)', borderTop: '1px solid var(--line)', borderBottom: '1px solid var(--line)' }}>
              ...
              <div className="mono" style={{ fontSize: 10.5, color: 'var(--ink-4)', letterSpacing: '.14em', marginTop: 4 }}>{r.date} · 2026</div>
            </div>
          ))}
        </div>
        ...
        <div data-reveal className="ticks" style={{ position: 'relative', border: '1px solid var(--line-2)', background: 'repeating-linear-gradient(135deg, rgba(255,255,255,0.018) 0 1px, transparent 1px 14px),linear-gradient(180deg, var(--bg-1), var(--bg))', minHeight: 380, padding: '60px 32px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 18, textAlign: 'center' }}>
          ...
          <div style={{ marginTop: 8, display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
            <Link href="/register" className="btn" style={{ padding: '12px 20px', fontSize: 12 }}>REGISTER →</Link>
        ...
        <style>{`@media (max-width:760px){.bracket-rail{grid-template-columns:repeat(2,1fr)!important}.bracket-rail .rail-cell:nth-child(2n){border-right:none!important}.bracket-rail .rail-cell{border-bottom:1px solid var(--line)}}`}</style>
      </div>
    </section>
  );
}
