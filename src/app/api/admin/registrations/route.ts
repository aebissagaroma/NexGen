...
export async function GET(req: Request) {
  ...
  if (q) {
    args.push(`%${q}%`);
    where.push(`(r.full_name ILIKE $${args.length} OR r.gamertag ILIKE $${args.length} OR r.phone ILIKE $${args.length})`);
  }
  if (club) {
    args.push(club);
  ...
  const rows = await query(
    `SELECT r.id, r.full_name, r.phone, r.gamertag, r.club_code, c.name AS club_name,
